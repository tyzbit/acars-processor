...
func (a ADSBExchangeAnnotator) GetDefaultFields() (s []string) {
	sap := FormatAsAPMessage(SingleAircraftPosition{}, a.Name())
	c := FormatAsAPMessage(ADSBExchangeCalculated{}, a.Name())
...
func (a ADSBExchangeAnnotator) Annotate(m APMessage) (APMessage, error) {
	...
	tailcode = goCommentToYAMLComment(tailcode)
	...
	apm := FormatAsAPMessage(pos, a.Name())
	...
	calcapm := FormatAsAPMessage(calc, a.Name())
