...
type ADSBExchangeCalculated struct {
	...
	AircraftGeolocation           string  `ap:"aircraft_geolocation"`
	...
	AircraftDistanceMi            float64 `ap:"aircraft_distance_mi"`
}
...
func (a ADSBExchangeAnnotator) Annotate(m APMessage) (APMessage, error) {
	...
	if tailcode == "" {
		log.Debug(Note("did not find a tail code in message"))
		return m, nil
	}
	pos, err := a.SingleAircraftPositionByRegistration(tailcode)
	if err != nil {
		log.Warn(Attention("error finding aircraft position: %v", err))
	}
	if len(pos.Aircraft) == 0 {
		return m, fmt.Errorf("no aircraft were returned from ADS-B API, response message was: %s", pos.Message)
	}
