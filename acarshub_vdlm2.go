	...
	VDL2                 struct {
		...
		AVLC struct {
			...
			ACARS     struct {
				...
				More                  bool   `json:"more"`
				Registration          string `json:"reg" acars:"tail_code"`
				...
				MessageText           string `json:"msg_text" acars:"message_text"`
			} `json:"acars" gorm:"embedded"`
		} `json:"avlc" gorm:"embedded"`
		BurstLengthOctets    int     `json:"burst_len_octets"`
		FrequencyHz          int     `json:"freq" acars:"frequency_hz"`
		Index                int     `json:"idx"`
		...
		SignalLevel          float64 `json:"sig_level" acars:"signal_level_dbm"`
		Station              string  `json:"station" acars:"station_id"`
		Timestamp            struct {
			UnixTimestamp int `json:"sec" acars:"unix_timestamp"`
