...
type DiscordReceiver struct {
	...
	RequiredFields []string `default:"[ACARSProcessor.MessageText]"`
}
...
func (d DiscordReceiver) Send(m APMessage) error {
	...
	for _, requiredField := range d.RequiredFields {
		if !slices.Contains(keys, requiredField) && m[requiredField] != "" {
