...
type BuiltinFilter struct {
	...
	// Require all of these regex strings to match or else filter the message. If the regex does not compile, the FilterOnFailure setting will be followed.
	RequireAllRegexMatches []string `jsonschema:"example=[.*LAV.*,.*COFFEE.*]" default:"[.*LAV.*,.*COFFEE.*]"`
	// Require at least a certain number of these regexes to match or else filter the message. If the regex does not compile, the FilterOnFailure setting will be followed.
