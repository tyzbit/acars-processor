...
func ReadACARSHubACARSMessages() {
	...
	for {
		...
		for {
			...
			log.Info(Content("new acars message received ending in \"", Emphasised(Last20Characters(next.MessageText)), Content("\"")))
			if (next == ACARSMessage{}) {
				...
				APMessageQueue <- APMessageQeueueItem{
					ACARSMessage: next,
					APMessage:    FormatAsAPMessage(next),
				}
				continue
			}
		}
...
func ReadACARSHubVDLM2Messages() {
	...
	for {
		...
		for {
			...
			log.Info(Content("new vdlm2 message received ending in \"", Emphasised(Last20Characters(next.VDL2.AVLC.ACARS.MessageText)), Content("\"")))
