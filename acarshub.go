	...
	for {
		...
		for {
			...
			log.Info(Content("new acars message received ending in \""),
				Emphasised(Last20Characters(next.MessageText)),
				Content("\""))
			if (next == ACARSMessage{}) {
				...
				nextap, err := next.Prepare(next)
				if err != nil {
					log.Error(Attention("error preparing acars message ending in \"",
						Emphasised(Last20Characters(next.MessageText)),
						Attention("\"")))
					continue
				}
	...
	for {
		...
		for {
			...
			log.Info(Content("new vdlm2 message received ending in \""),
				Emphasised(Last20Characters(next.VDL2.AVLC.ACARS.MessageText)),
				Content("\""))
			if (next == VDLM2Message{}) {
				...
				nextap, err := next.Prepare(next)
				if err != nil {
					log.Error(Attention("error preparing vdlm2 message ending in \"",
						Emphasised(Last20Characters(next.VDL2.AVLC.ACARS.MessageText)),
						Attention("\"")))
					continue
				}
