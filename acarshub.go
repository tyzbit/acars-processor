...
func ReadACARSHubACARSMessages() {
	...
	for {
		...
		for {
			...
			log.Info(Content("new acars message received ending in \"", Emphasised(next.MessageText), Content("\"")))
...
func ReadACARSHubVDLM2Messages() {
	...
	for {
		...
		for {
			...
			log.Info(Content("new vdlm2 message received ending in \"", Emphasised(next.VDL2.AVLC.ACARS.MessageText), Content("\"")))
