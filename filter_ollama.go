...
func (o OllamaFilterer) Filter(m APMessage) (filter bool, reason string, err error) {
	...
	defer cancel()
	log.Debug(Emphasised("calling OllamaFilter for message ending in \""),
	...
	log.Debug(Emphasised("Ollama decision: "),
		action[r.MessageMatchesCriteria],
		Emphasised(" for message ending in \""),
		Note(Last20Characters(ms)),
		Emphasised("\", reasoning: "),
		Note(r.Reasoning))
